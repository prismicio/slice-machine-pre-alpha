  ...
  >
    <div class="ps__wrap">
      <div class="ps__head">
        ...
        <div class="ps__desc">
          <p>
            These are some awesome videos that teach you how to use Prismic
            Slices to quickly create a great Web site.
          </p>
        </div>
      </div>
      <div class="ps__main">
        <div :id="currId" class="ps__video-player" data-video-player>
                ...
                <a
                  ...
                  :class="
                    `ps__video-player__playlist__link ${
                      currentItem.index === index && currentItem.isLoading
                        ? 'video-loading'
                        : ''
                    }`
                  "
          ...
          <div
            :aria-labelledby="`${currId}__tab-${currentItem.index}`"
            class="ps__video-container span-1-8"
            :tabindex="videoTabIndex"
            @blur="onContainerBlur"
          >
            <!-- <prismic-embed :field="dataItems.find(e => e.selected).src" /> -->
            <iframe
              title="Video player"
              width="560"
              height="315"
              :src="dataItems.find(e => e.selected).src.embed_url"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              @load="setLoadingState(false)"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    console.log('HERE', this.slice.items[0].video_src)
    return {
      videoTabIndex: '-1',
      currentItem: {
        index: 0,
  ...
  methods: {
    selectItem(index) {
      if (this.currentItem.index === index) {
        return
      }
      this.currentItem = { index, isLoading: true }
...
.ps__video-player__playlist__link {
  ...
  &[aria-selected='true'].video-loading {
    background-image: url('./video-load-icon--black.svg');
    font-weight: bold;
  ...
  .ps-video-player--dark &[aria-selected='true'].video-loading {
    background-image: url('./video-load-icon--black.svg');
    font-weight: bold;
    background-size: 34px;
    background-position-x: 101%;
