...
(function (w, doc, undefined) {
  var ARIAaccOptions = {
    ...
    icon: '<svg aria-hidden="true" focusable="false" width="50" height="50" viewBox="0 0 50 50" class="accordion-icon"><circle stroke-width="2" fill="transparent" stroke="currentColor" cx="16" cy="16" r="16" transform="translate(9 9)"/><path stroke-width="2" stroke="currentColor" fill="none" d="M20 23l5.5 5.092 5.5-5.092" /></svg>'
    /**
     * ARIA Accordion
  ...
  var ARIAaccordion = function ARIAaccordion(inst, options) {
    ...
    var setupAccordionPanels = function setupAccordionPanels(accordionPanels) {
      Array.from(accordionPanels).forEach(function (item, index) {
        var $this = item;
        $this.setAttribute("id", "c-accordion-panel-" + index); // $this.setAttribute('aria-labelledby', "c-accordion-heading-" + index);
    ...
    var togglePanel = function togglePanel(toggleButton) {
      ...
      if (toggleButton.getAttribute("aria-expanded") == "true") {
        ...
        if (_options.showOneAnswerAtATime) {
          ...
          accordionHeadings.querySelector("button").setAttribute("aria-expanded", "false");
          checkToggleCollapseButtonState();
          checkToggleExpandButtonState();
        } // Show answer
    ...
    var checkToggleExpandButtonState = function checkToggleExpandButtonState() {
      var closedPanels = el.querySelectorAll('button[aria-expanded="false"]'); // find any and all collapsed panels
    ...
    var checkToggleCollapseButtonState = function checkToggleCollapseButtonState() {
      var openPanels = el.querySelectorAll('button[aria-expanded="true"]'); // find any and all expanded panels
...
for (var i = 0; i < els.length; i++) {
  var nAccs = new ARIAaccordion(els[i]); // var nAccs = new ARIAaccordion(els[i], { withControls: false });
